...
class Primer_Temp():
	...
	def _edit_distance(self, string_x=None, string_y=None, get_backtrace=True):
		...
		if get_backtrace:
			...
			self.cigar = self._align_gRNA(string_x, string_y, bt)
	...
	def _align_gRNA(self, string_x, string_y, bt, sensitive_region = (0,5)):
		'''
...
def bitapSearch(reference, query, reverse = False, mismatch = 10, best = False):
	#if reverse: query = reverseC(query)
	...
	for k in range(0, mismatch + 1):
		...
		for columnNum in range(1, referenceLen + 1):
			...
			if (curColumn & 0x1) == 0:
				...
				if len(place) > 4: 
					temp = Primer_Temp(query, place, (startPos, endPos), k - 1)
					primer.append(temp)
					
					if best: return gRNAs
					skip.append(startPos)
				else:
					primer.append(startPos)
